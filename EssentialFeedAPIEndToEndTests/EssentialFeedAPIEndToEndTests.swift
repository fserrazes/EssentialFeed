...
class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    func test_end_to_end_server_get_feed_result_matches_fixed_test_account_data() {
        switch getFeedResult() {
            case let .success(imageFeed)?:
                XCTAssertEqual(imageFeed.count, 8, "Expected 8 images in this test account image feed")
    ...
    private func getFeedResult(file: StaticString = #filePath, line: UInt = #line) -> Swift.Result<[FeedImage], Error>? {
        ...
        var receiveResult: Swift.Result<[FeedImage], Error>?
        client.get(from: feedTestServerURL) { result in
            receiveResult = result.flatMap { (data, response) in
                do {
