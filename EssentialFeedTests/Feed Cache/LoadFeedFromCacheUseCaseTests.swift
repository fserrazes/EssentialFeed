...
class LoadFeedFromCacheUseCaseTests: XCTestCase {
    ...
    func test_load_delivers_cached_images_on_less_than_seven_days_old_cache() {
    ...
    func test_load_delivers_no_images_on_seven_days_old_cache() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
    ...
    func test_load_delivers_no_images_on_more_than_seven_days_old_cache() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
    ...
    func test_load_deletes_cache_on_retrieval_error() {
        let (sut , store) = makeSUT()
    ...
    func test_load_does_not_deletes_cache_on_empty_cache() {
        let (sut , store) = makeSUT()
    ...
    func test_load_deletes_cache_on_seven_days_old_cache() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
        let sevenDaysOldTimestamp = fixedCurrentDate.adding(days: -7)
        let (sut , store) = makeSUT(currentDate: { fixedCurrentDate })
    ...
    func test_load_deletes_cache_on_more_than_seven_days_old_cache() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
        let moreThanSevenDaysOldTimestamp = fixedCurrentDate.adding(days: -7).adding(seconds: -1)
        let (sut , store) = makeSUT(currentDate: { fixedCurrentDate })
